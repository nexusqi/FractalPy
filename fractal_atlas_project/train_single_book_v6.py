# train_single_book_v6.py
# Обучение FractalDecoder V6 на одной "мастер-книге".
#
# Результат:
#   - файл fractal_text_decoder_v6.pt
#     в нём: веса модели, словарь stoi/itos и конфиг.
...
from .fractal_decoder_v6 import FractalDecoderV6
...
def decode_full_text(model, dataset: TextFractalDataset) -> str:
    ...
    with torch.no_grad():
        for i in range(len(dataset)):
            ...
            logits = model(x_in)
            pred_idx = torch.argmax(logits, dim=-1).item()
            chars.append(dataset.itos[pred_idx])
    return "".join(chars)
...
def main():
    ...
    # Конфиг V6 (записываем в чекпоинт)
    config = {
        # Fourier
        "fourier_frequencies": 32,
        "max_fourier_freq": 1e4,   # как просил Саша: до sin(10000 x)
        # Hash-grid
        "hash_num_levels": 8,
        "hash_min_res": 16,
        "hash_max_res": 2048,      # можно даже выше, но начнём так
        ...
        # Latent key
        "latent_dim": 32,
        # Sparse updates по умолчанию ВЫКЛЮЧЕНЫ
    ...
    for epoch in range(1, num_epochs + 1):
        ...
        for x_batch, y_batch in dataloader:
            logits = model(x_batch)  # z по умолчанию нулевой
            ce_loss = criterion(logits, y_batch)

            # Простая регуляризация весов
    ...
    ckpt_path = os.path.join(here, "fractal_text_decoder_v6.pt")
    torch.save(
        {
    ...
    print(f"Модель V6 сохранена в: {ckpt_path}")
